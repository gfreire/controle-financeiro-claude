...
export function FixedExpenseFormDialog({ categories, accounts }: { categories: CategoryDTO[]; accounts: AccountDTO[] }) {
  ...
  const [defaultAccountId, setDefaultAccountId] = useState(NONE);
  ...
  function handleSubmit() {
    ...
    startTransition(async () => {
      try {
        await createFixedExpenseAction(parsed.data);
        router.refresh();
        setOpen(false);
        setName(""); setAmount("");
      } catch (e) {
        setError(e instanceof Error ? e.message : "Erro ao criar despesa fixa");
      }
    });
  }
