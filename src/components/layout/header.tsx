...
export function Header({ userName, userEmail }: { userName: string | null; userEmail: string | null }) {
  return (
    <header className="flex items-center justify-between border-b border-divider px-4 py-3 md:px-6">
      ...
      <div className="hidden text-sm opacity-70 md:block">{userName || userEmail}</div>
      <form action={signOut}>
        <button type="submit" className="flex items-center gap-1.5 p-1.5 -m-1.5 text-xs text-text/60 hover:text-accent">
