...
export function BottomNavigation() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 flex border-t border-divider bg-bg md:hidden">
      {items.map((item) => {
      ...
      <Popover>
        ...
        <PopoverContent side="top" align="end" className="mb-2 flex w-44 flex-col gap-0.5">
          {SECONDARY_NAV.map((item) => {
            const Icon = item.icon;
            return (
              <Link key={item.href} href={item.href} className="flex items-center gap-2.5 px-2 py-2 text-sm hover:bg-text/[0.06]">
