...
- **`is_system` EXPENSE category "Pagamento de Cartão" + backfill (migration `0031`, 2026-08-28, at the user's request).** Every `CREDIT_CARD_PAYMENT` transaction is now tagged with a dedicated `is_system` EXPENSE category — automatically by `cards.service.ts#registerCardPayment` (via the new `getCardPaymentCategoryId` helper), and backfilled onto all existing ones. The user can never select it (`is_system` ⇒ filtered from `CategorySelect`); `EditableCategoryCell` still shows these rows as the fixed "Pagamento de Cartão" text (early-return on `row.type`), so the Explorer display is unchanged. It's EXPENSE-typed despite `CREDIT_CARD_PAYMENT` not being an EXPENSE transaction (no `CategoryType` for it — the user chose EXPENSE explicitly) and never enters analytics — `fetchPeriodEntries`/`getTransactionsFiltered`/`getFinancialSummary` all restrict `type in ('INCOME','EXPENSE')`. Where it's useful: `/transactions`' category filter lists `is_system` categories, so filtering by "Pagamento de Cartão" there groups the card-bill payments; and it opens room for future reports. See `AI_CONTEXT.md` → "Pagamento de Cartão — categoria `is_system`".
...
- **"Tipo" dropdown folded into the category filter; "Despesas de {mês}" card hidden under a category filter (2026-08-28, at the user's request).** Two dashboard-filter tweaks, presentation-only, no service/DTO/schema change:
  - The standalone "Tipo" (Receita e despesa / Receitas / Despesas) `Select` in `dashboard-filters.tsx` was **removed**. The category filter popover (`CategoryCheckboxFilter`) now renders a select-all checkbox on each group header ("Receitas" / "Despesas") when the caller passes the new optional `onToggleGroup` prop — `CategoryMultiSelect` wires it to `useCategoryFilter().setGroup` (bulk add/remove of a group's ids in one navigation). Checkbox is tri-state: checked when all of a group's categories are selected, indeterminate when some, off when none; unchecking one member leaves the rest selected + the group indeterminate; re-checking the group re-selects all. `Checkbox` (`src/components/ui/checkbox.tsx`) gained an indeterminate glyph (`Minus`, via `group-data-[state=indeterminate]`). `parseDashboardFilters` no longer reads `?type=` — `transactionType` is always `undefined` from the URL now (the two category donuts in `page.tsx` still set it per-call). The Cards evolution filter passes no `onToggleGroup`, so it's unchanged. Not a perfect equivalent to the old `transactionType` (all-categories-of-a-type excludes *uncategorized* rows of that type) — accepted as a UI simplification.
  - `MonthObligationsCard` ("Despesas de {mês}") is **not rendered when a category/subcategory/`uncategorizedOnly` filter is active** (`dashboard/page.tsx`, reusing the existing `hasCategoryFilter`). It lists whole-month commitments that aren't category-scoped, so showing it beside an already-category-filtered page was confusing. `getCurrentMonthObligations` is still called in the page's `Promise.all` regardless (a small wasted fetch under a filter — not worth restructuring the batch to skip).

- **Dívidas split into three screens (2026-08-29, at the user's request — "só separar forms e menus para ser mais intuitivo").** The one "Dívidas" screen (`/debts`) holding all three `debts.kind` values became three: `/debts` → "Dívidas Pessoais" (`PERSONAL`, keeps the side filter + both pies), `/overdue-bills` → "Contas em Atraso" (`OVERDUE_BILL`), `/installment-plans` → "Parcelamento Programado" (`INSTALLMENT_PLAN`). **Presentation only** — service, DTOs, `debts.*` validations, the `debt_transactions`/linked-transaction pipeline, and the dashboard projection (`getCurrentMonthObligations`, `fetchUnpaidObligationEntries`, `MonthObligationsCard`) are untouched. `DebtFormDialog` gained a `kind` prop (fixed by the screen in create mode, from `debt.kind` in edit mode) and **lost its "Tipo de dívida" selector** — changing a debt's kind via the UI is no longer possible (each kind has its own screen). New shared `features/debts` pieces: `debts-view.tsx` (async — filters `getDebts()` by kind + optional side, renders charts + list) and `debt-card.tsx` (async — one card, fetches its own ledger). `DebtsCharts` gained `payableTitle`/`receivableTitle` props. `revalidateDebtPaths()` in `features/debts/actions.ts` now revalidates all three routes. `DebtSideFilter` stays on `/debts` only. See `AI_CONTEXT.md` → "Dívidas — subtipos" → "Telas separadas por `kind`".
- **Parcelamento Programado gained a start competence + adiantado/atrasado (migration `0032`, 2026-08-29, at the user's request).** `INSTALLMENT_PLAN` debts now carry `debts.start_competence` (a "YYYY-MM" month, required in zod for this kind). Payments are attributed to competence months **automatically, oldest-first** (no per-payment picker, no new `debt_transactions` column) — the exact `currentMonthPaidAmount` heuristic the card invoice already uses; paying two boletos today covers the next two competences and carries credit forward (nothing is removed, unlike the card "Antecipar parcelas" flow). `getDebts()` derives three new `DebtDTO` fields: `startCompetence`, `paidThroughCompetence` (last competence covered), `scheduleOffset` (signed months ahead/behind, today-anchored, capped at the plan's total installment count). `debt-card.tsx` shows an "Adiantado N meses" / "Atrasado N meses" / "Em dia" badge plus "Pago até {mês}", replacing the old "Pago este mês / Vence dia X" pair. `DebtFormDialog` gained a "Mês de competência inicial" `<input type="month">` in the INSTALLMENT_PLAN block. The dashboard's presentation is unchanged, but `getCurrentMonthObligations`/`fetchUnpaidObligationEntries` now gate an INSTALLMENT_PLAN obligation by competence (`month >= startCompetence` and `month` not covered) instead of by calendar-month `paidThisMonth` — which makes the "Despesas do mês" card correct for any browsed month and retires the documented "INSTALLMENT_PLAN `paidThisMonth` is today-anchored" caveat (only `OVERDUE_BILL`, competence-less, keeps its version). `paidThisMonth` stays in the DTO with no UI consumer. See `AI_CONTEXT.md` → "Parcelamento Programado — competência e adiantado/atrasado".
...
31. `0031_card_payment_category.sql` — data-only: adds one `is_system` **EXPENSE** category "Pagamento de Cartão" and backfills `category_id` on every existing `CREDIT_CARD_PAYMENT` transaction (guarded `category_id is null`). `cards.service.ts#registerCardPayment` (via `insertCardPayment`) now tags every new card-bill payment with it automatically — the user can never pick it (`is_system` ⇒ out of `CategorySelect`), it's a label only. EXPENSE-typed even though `CREDIT_CARD_PAYMENT` isn't an EXPENSE transaction (no `CategoryType` for it — an explicit user choice) and it never reaches analytics (the queries restrict `type in ('INCOME','EXPENSE')`). Like `0017`/`0018`/`0030`, a pure catalog `INSERT` + data `UPDATE` — no schema-cache reload. Historical note: migration `0006` *removed* a `is_default` **subcategory** of the same name for inviting the wrong entry method; this `is_system` category is the opposite — never user-chosen. See `AI_CONTEXT.md` → "Pagamento de Cartão — categoria `is_system`", decided 2026-08-28 at the user's request.
32. `0032_debt_installment_start_competence.sql` — adds `debts.start_competence` (`date`, nullable — only meaningful for `INSTALLMENT_PLAN`, required-in-zod-not-in-CHECK, same convention as `monthly_amount`/`due_day` from `0021`). Backfills existing `INSTALLMENT_PLAN` rows with `date_trunc('month', created_at)` (the kind is 6 days old, so the creation month is a safe default). Lets the system tell whether a parcelamento is ahead/behind schedule and judge "paid" by competence month (oldest-first allocation, the card-invoice heuristic) instead of by calendar month. See `AI_CONTEXT.md` → "Parcelamento Programado — competência e adiantado/atrasado", decided 2026-08-29 at the user's request.
33. `0033_reload_schema_cache.sql` — `NOTIFY pgrst, 'reload schema'`, same reasoning as `0020`/`0022`/`0024`/`0027`/`0029`, for `debts.start_competence`.
34. `0034_goal_transaction_types.sql` — adds `RESERVE` / `REDEEM` to the `transaction_type` enum (feature "Metas"). Isolated in its own file (not used as a literal in any DDL here) to avoid Postgres' "unsafe use of new enum value" when a migration adds and uses a value in one transaction. Aporte/resgate de uma Meta — move only account balances, never INCOME/EXPENSE. See `AI_CONTEXT.md` → "Metas", decided 2026-08-30 at the user's request.
35. `0035_goals.sql` — `goals` (`name`, `goal_target` NOT NULL > 0, `start_competence`, `end_date?`, `monthly_contribution?`, `anchor_date`) + `goal_yields` (`amount` > 0, `date`, `origin_redeem_transaction_id?` — NULL = informed yield, set = recognized inside a redeem, FK `ON DELETE CASCADE` to that REDEEM) + `transactions.goal_id` (FK `ON DELETE SET NULL`). RLS `auth.uid() = user_id` on both new tables. `currentBalance` never stored — `Σ RESERVE − Σ REDEEM + Σ goal_yields`.
36. `0036_goal_redeem_categories.sql` — data-only: 2 `is_system` **INCOME** categories, "Resgate de Meta Concluída" / "Resgate de Meta Antecipado" — the label a REDEEM carries, auto-picked from balance vs. target. Like `0031`, never user-pickable and never in analytics (REDEEM excluded by `type`); only a `/transactions` filter handle. No schema-cache reload needed for a pure catalog INSERT.
37. `0037_reload_schema_cache.sql` — `NOTIFY pgrst, 'reload schema'`, for the `goals`/`goal_yields` tables + `transactions.goal_id` from `0035`.
...
src
 ...
 ├ services (one per domain: dashboard, transactions, accounts, categories, cards, reservoirs, goals, debts, budgets, fixed-expenses, profile; _shared.ts holds the budget/fixed-expense actualAmount aggregation both reuse)
 ├ features
 ...
 │  ├ goals/components (feature "Metas", migrations 0034-0037: goal-form-dialog [create+edit; month pickers for start/end; live "pra concluir até X: ~R$/mês" suggestion; optional initial reserve on create], reserve-dialog [aporte], redeem-dialog [resgate — Concluída/Antecipado toggle, prefilled by balance vs. target], goal-yield-dialog [create="Informar rendimento" delta / edit an informed YIELD row], edit-goal-entry-dialog [edit a RESERVE/REDEEM], recalculate-goal-button [rebase — only when the goal has a deadline], delete-goal-button, delete-goal-entry-button [RESERVE/REDEEM or YIELD], goal-card.tsx [async — donut + status badges + Aportar/Rendimento/Resgatar/Recalcular + ledger list], goal-accumulation-chart.tsx [/goals "Acumulado guardado" — 13-month cumulative bar + ReferenceLine at total target]) — dashboard block is features/dashboard/components/goals-overview.tsx
 │  ├ debts/components (shared by /debts, /overdue-bills, /installment-plans — split 2026-08-29, presentation only: debts-view.tsx [async — filters getDebts() by a fixed `kind` (+ optional side for PERSONAL), renders charts + list of DebtCard], debt-card.tsx [async — one debt card, fetches its own getDebtTransactions], debt-form-dialog [create+edit, `kind` fixed by the screen (create) or debt.kind (edit) — NO kind selector anymore; still shows the direction picker only for PERSONAL and monthlyAmount/dueDay/startCompetence (month picker, migration 0032) only for INSTALLMENT_PLAN; incl. default category], debt-transaction-dialog [create+edit, defaults description to "Movimentação da dívida {nome}"; optional interest-percentage calculator on mode="increase" (suggests amount = currentBalance × %, still editable); accepts a `defaultAmount` prop for quick-pay triggers; warns and requires a second confirm before a payment that fully settles/overpays the debt; editing propagates to the linked transaction, direction locked], delete-debt-button [manual soft delete — forgiven/given-up-on debt], delete-debt-transaction-button [deletes the linked transaction too], debt-side-filter.tsx ["Todas/A pagar/A receber", /debts only], debts-charts ["Dívidas a pagar"/"Dívidas a receber" pies (titles overridable via payableTitle/receivableTitle), each rendered only when that side has data])
 ...
 └ app
    ...
    └ (app)/dashboard, transactions, accounts, cards, reservoirs, goals, debts, overdue-bills, installment-plans, budgets, settings — layout.tsx here redirects to /onboarding whenever profiles.onboarding_completed is false; every route also has its own loading.tsx (see loading-overlay above). debts / overdue-bills / installment-plans are the three `debts.kind` values split into dedicated screens 2026-08-29 (presentation only — same service/DTOs/rules), all three thin pages over the shared `features/debts` `DebtsView`/`DebtCard`.
```
...
(app)
 ...
 cards/page.tsx
 reservoirs/page.tsx
 goals/page.tsx             ("Metas" — feature `goals`, migrations 0034-0037)
 debts/page.tsx              ("Dívidas Pessoais" — debts.kind = PERSONAL)
 overdue-bills/page.tsx      ("Contas em Atraso" — debts.kind = OVERDUE_BILL)
 installment-plans/page.tsx  ("Parcelamento Programado" — debts.kind = INSTALLMENT_PLAN)
 budgets/page.tsx        (inclui despesas programadas)
 settings/page.tsx
```

Navigation: mobile-first, bottom navigation (Dashboard, Transactions, Accounts, Cards, More), desktop sidebar (adds Receita Programada [`/reservoirs` — see AI_CONTEXT.md "Reservoir (Cofre)" for why the route/table names stay `reservoir*` while the UI label doesn't], Metas [`/goals`], Dívidas Pessoais, Contas em Atraso, Parcelamento Programado, Budgets, Settings). The three debt screens were one "Dívidas" screen until 2026-08-29 — split for clarity, no rule change (see AI_CONTEXT.md "Dívidas — subtipos" → "Telas separadas por kind").
...
type DashboardFilters = {
  ...
  transactionType?: "INCOME" | "EXPENSE" // no longer a URL control (2026-08-28) — parseDashboardFilters always sets it undefined; only dashboard/page.tsx's two category donuts still set it per-call. "Só receitas" / "só despesas" is now done by checking a whole "Receitas"/"Despesas" group in the category filter.
...
The category filter popover (`CategoryCheckboxFilter`, shared with the Cards evolution chart) gives the "Receitas" and "Despesas" group headers a select-all checkbox (checked / indeterminate / off) when the caller passes `onToggleGroup` — the dashboard does, the Cards page doesn't. Checking a group adds every category id of that type to `categories`; unchecking one member afterwards just drops that id, leaving the group checkbox indeterminate with the rest selected; re-checking the group re-selects all. This replaced the standalone "Tipo" Select. Semantics aren't identical to the old `transactionType` (selecting all categories of a type doesn't pull in *uncategorized* rows of that type) — accepted as a UI simplification at the user's request.
...
3. Category Distribution, EXPENSE (donut, segmented by account type via `source` above) and INCOME (mirrored) — both render side by side (each donut forces its own `transactionType` per-call in `page.tsx`, so they don't depend on any global type control — and there is no global type control anymore, see the filter note above). Exception (2026-08-28): when a **category/subcategory filter is active** and it produces no data for one side while the other side has data, that empty side's whole block (heading + `ExpenseSourceToggle` + donut) is not rendered — filtering by an expense-only category shouldn't leave a "Sem lançamentos" receitas donut sitting there, and vice-versa. Gated so at least one donut (with its "limpar filtro" affordance) always survives; the no-filter empty state still shows the informative empty card. Done in `dashboard/page.tsx`. The horizontal-bar "comparison" chart that used to sit next to each donut was removed 2026-08-28 (it duplicated the donut's data — see Implementation Status). The EXPENSE donut also shows the viewed month's unpaid projected obligations as slices on their real category (2026-08-28), unless the account filter or the liquid/cards source toggle is active.
...
getFinancialSummary(filters, obligationsMonth?) → FinancialSummaryDTO
  -- reservedTotal (NOVO 2026-08-30) = getReservedTotal() (Σ saldo de todas as metas) — sub-linha
  -- "guardado em metas" no card de Saldo. Figura global, ignora o filtro de conta.
getMonthlyEvolution(filters, obligationsMonth?) → MonthlyEvolutionDTO[]
  ...
  -- `reserved` por mês (NOVO 2026-08-30) = Σ RESERVE − Σ REDEEM (goal-linked, respeita o filtro
  -- de conta) datado no mês — a 3ª barra "Guardado (metas)", um fluxo mensal (não cumulativo).
  -- fetchPeriodEntries também emite as goal_yields do período como INCOME sintético sob
  -- "Rendimentos" (income-side only, pula uncategorized/subcategoria/filtro-de-conta).
...
getCurrentMonthObligations(month?) → MonthObligationsDTO
  ...
  -- 2026-08-28 mesmo dia, antes era sempre o mês real corrente. INSTALLMENT_PLAN agora é filtrado
  -- por competência (startCompetence/paidThroughCompetence, migration 0032), correto pra qualquer
  -- mês; só OVERDUE_BILL (sem competência) aparece em qualquer mês. `total` = `paidTotal`
...
registerCardPayment(data)
  -- cria a transaction CREDIT_CARD_PAYMENT + a card_payments vinculada. description default
  -- "Pagamento da fatura do cartão {nome}". NOVO (0031): a transaction nasce com category_id =
  -- categoria is_system EXPENSE "Pagamento de Cartão" (helper getCardPaymentCategoryId), aplicada
  -- automaticamente — o usuário nunca escolhe. É só rótulo: analytics restringem type in
  -- ('INCOME','EXPENSE'), então nunca puxa esse pagamento pra um gráfico/soma. Ver AI_CONTEXT.md
...
## goals.service.ts (NOVO — feature "Metas", migrations 0034-0037)
```
-- Espelho invertido da Receita Programada. currentBalance = Σ RESERVE − Σ REDEEM + Σ goal_yields,
-- sempre calculado. Aporte/resgate = transactions RESERVE/REDEEM (só CASH/BANK, checado
-- server-side; movem saldo de conta, nunca INCOME/EXPENSE). Rendimento = goal_yields (RECEITA
-- sintética sob "Rendimentos" no dashboard). Ver AI_CONTEXT.md "Metas".
createGoal(data) → id   -- data.endDate sem monthlyContribution → calcula o aporte = (alvo −
  -- reservaInicial) / meses. data.initialReserve{AccountId,Amount} opcional cria um 1º RESERVE
  -- datado em start_competence (entra no anchorBalance, não conta como aporte posterior)
updateGoal(id, data)   -- partial. rebase: true (botão "Recalcular") OU mudar endDate →
  -- anchor_date = hoje e, sem monthlyContribution explícito, regrava o aporte a partir do saldo
  -- atual. O ledger NUNCA é tocado — "quanto já foi feito" = saldo calculado em anchor_date
deleteGoal(id)   -- hard delete. goal_yields cascateia; RESERVE/REDEEM sobrevivem com goal_id NULL
addReserve(data)   -- aporte: transactions RESERVE, origin = conta, description default "Aporte
  -- para meta {nome}"
redeemGoal(data)   -- resgate: 1 transactions REDEEM pelo valor CHEIO (destination = conta,
  -- category = "Resgate de Meta Concluída"/"Antecipado" — auto por saldo vs. alvo, sobrescrevível
  -- por toggle). Se amount > saldo de livro, o excedente vira goal_yields datada no dia,
  -- origin_redeem_transaction_id = o REDEEM (cascateia ao apagá-lo)
registerGoalYield(data)   -- "Informar rendimento": delta entre realBalance e o saldo calculado
  -- → goal_yields (origin_redeem_transaction_id NULL). Só delta positivo na v1
updateGoalEntry(data)   -- edita um RESERVE/REDEEM (valor/data/conta/descrição), propaga pra
  -- transactions. Direção (aporte/resgate) nunca muda. Não recalcula o goal_yields reconhecido
  -- de um REDEEM editado — pra mudar o valor de um resgate assim, exclua e refaça
updateGoalYield(data) / deleteGoalYield(id)   -- deleteGoalYield só um rendimento INFORMADO
  -- (origin_redeem_transaction_id NULL); um reconhecido se apaga apagando o resgate
deleteGoalEntry(id)   -- hard delete de um RESERVE/REDEEM (lançamento espúrio). Pra typo, o
  -- caminho é editar. Um goal_yields reconhecido num REDEEM apagado cascateia
getGoals() → GoalDTO[]   -- currentBalance + cronograma (status/scheduleOffsetMonths/expectedByNow/
  -- projectedCompletionMonth, padrão INSTALLMENT_PLAN 0032, hoje-ancorado). 2 queries totais
  -- (todos os RESERVE/REDEEM + todos os goal_yields do usuário, agrupados em memória)
getGoalEntries(goalId) → GoalEntryDTO[]   -- RESERVE/REDEEM (com nome da conta) + goal_yields,
  -- merge ordenado por data desc. withdrawalReason derivado da categoria system do REDEEM
getReservedTotal() → number   -- Σ saldo de todas as metas vivas (exclui RESERVE/REDEEM órfãos de
  -- meta apagada). Alimenta FinancialSummaryDTO.reservedTotal — figura global, não escopada pelo
  -- filtro de conta
getGoalsOverview() → GoalsOverviewDTO   -- figuras compactas por meta pro bloco do dashboard
getGoalAccumulation() → GoalAccumulationDTO   -- total guardado no fim de cada um dos últimos 13
  -- meses (cumulativo — um estoque) + soma dos alvos; alimenta o gráfico de /goals
```
...
createDebt(data)   -- data.defaultCategoryId (NOVO 0015) opcional, tipo EXPENSE (PAYABLE) ou
  ...
  -- "Dívidas — subtipos". monthlyAmount/dueDay/startCompetence (NOVO 0032, "YYYY-MM", normalizado
  -- pro 1º dia do mês) obrigatórios (validados em zod) só quando kind === INSTALLMENT_PLAN
updateDebt(id, data)  -- NOVO (0016), estendido 2026-08-23/29: partial update de agent/side/kind/
  -- initialBalance/defaultCategoryId/monthlyAmount/dueDay/startCompetence — todos livremente
  -- editáveis depois de criada a dívida; só a fórmula do remainingBalance (initial_balance +
...
getDebts() → DebtDTO[]   -- filtra active = true, então uma dívida quitada já não aparece.
  -- INSTALLMENT_PLAN, tudo derivado do ledger + monthly_amount + start_competence (NOVO 0032),
  -- sem coluna nova: paidThroughCompetence = start_competence + (floor(totalPago / monthlyAmount)
  -- em centavos − 1) meses — alocação automática mais-antigo-primeiro, mesma heurística da fatura
  -- de cartão (currentMonthPaidAmount); scheduleOffset = coberto − esperado (esperado = meses de
  -- start_competence até hoje, teto no total de parcelas) → adiantado(+)/atrasado(−)/em dia(0).
  -- paidThisMonth ainda calculado mas virou sinal secundário — nenhuma UI usa. Ver AI_CONTEXT.md
...
type FinancialSummaryDTO = {
  ...
  reservedTotal: number // NOVO (2026-08-30) — Σ current balance of every Meta (Σ RESERVE − Σ REDEEM + Σ goal_yields); "guardado em metas" sub-line on the Saldo card. Global, not scoped by the account filter. See AI_CONTEXT.md "Metas".
}
...
// `reserved` (NOVO 2026-08-30) = net Σ RESERVE − Σ REDEEM (Meta aporte/resgate) dated in that
// month — a monthly FLOW (same unit as income/expense), the 3rd bar. Not cumulative — the
// running total lives in /goals' own "Acumulado guardado" chart. See AI_CONTEXT.md "Metas".
...
// Metas (feature "Goals", migrations 0034-0037). currentBalance = Σ RESERVE − Σ REDEEM +
// Σ goal_yields, nunca coluna. Cronograma segue o padrão INSTALLMENT_PLAN (0032), hoje-ancorado.
...
type GoalDTO = {
  ...
  anchorDate: string                  // "YYYY-MM-DD" — início da perna atual do cronograma (= startCompetence, ou uma data de rebase)
  endDate?: string                    // "YYYY-MM"
  monthlyContribution?: number        // aporte mensal planejado; ausente ⇒ NO_SCHEDULE (só tracker de progresso)
  status: GoalStatus
  scheduleOffsetMonths?: number       // meses com sinal: > 0 adiantado, < 0 atrasado; só com monthlyContribution e status != REACHED
  expectedByNow?: number              // anchorBalance + monthlyContribution × meses desde a âncora, teto no alvo
  projectedCompletionMonth?: string   // "YYYY-MM" — ceil((alvo − saldo) / aporte) meses à frente
}
type GoalEntryDTO = {
  id: string; kind: "RESERVE" | "REDEEM" | "YIELD"
  date: string; description: string | null
  amount: number                      // sempre positivo — `kind` carrega a direção
  accountId?: string; accountName?: string  // RESERVE/REDEEM
  withdrawalReason?: "COMPLETED" | "EARLY"  // REDEEM — derivado da categoria system vinculada
}
type GoalsOverviewDTO = { goals: { id; name; goalTarget; currentBalance; progressPercent; status: GoalStatus; scheduleOffsetMonths? }[] }
type GoalAccumulationDTO = { points: { month: string; total: number }[]; targetTotal: number } // /goals "Acumulado guardado" — 13 meses, cumulativo
...
type DebtDTO = {
  ...
  startCompetence?: string // NOVO (0032) — só INSTALLMENT_PLAN, "YYYY-MM", mês a partir do qual o parcelamento conta
  paidThroughCompetence?: string // NOVO (0032) — só INSTALLMENT_PLAN, "YYYY-MM", última competência coberta pelos pagamentos (mais-antigo-primeiro); ausente quando nada foi pago
  scheduleOffset?: number // NOVO (0032) — só INSTALLMENT_PLAN, meses com sinal: >0 adiantado, <0 atrasado, 0 em dia
  paidThisMonth?: boolean // NOVO (0021) — só INSTALLMENT_PLAN, pagamento datado no mês corrente. Sinal secundário desde 0032 — nenhuma UI usa; startCompetence/paidThroughCompetence/scheduleOffset é o que a UI lê
}
