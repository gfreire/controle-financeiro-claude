...
just another entry in the shared set, so it flows into `getCategoryDistribution`/
`getCategoryComparison` too, not only the summary/evolution sums. Not added to
`getTransactionsFiltered` (no real transaction row, same as `card_refunds`).
