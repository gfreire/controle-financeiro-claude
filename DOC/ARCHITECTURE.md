...
src
 ...
 ├ features
 │  ├ dashboard/components (dashboard-filters incl. shared MonthPicker + account-type icons per account + category-multi-select.tsx for the additive category filter, use-category-filter.ts, filters.ts [parseDashboardFilters], month-obligations-card.tsx ["Despesas de {mês}" — donut (`total` no centro = realizado + a pagar; fatia Pago + 1 fatia por item não pago) + lista acionável do que falta pagar; cartão por competência (getCardSummary), sempre mês real; faturas de cartão + despesas programadas + dívidas OVERDUE_BILL/INSTALLMENT_PLAN, cada linha com dialog de pagamento; substituiu upcoming-due-alert.tsx em 2026-08-28], open-debts-alert.tsx ["Dívidas em aberto" — OVERDUE_BILL/INSTALLMENT_PLAN debts, always shown regardless of period filter, see AI_CONTEXT.md "Dívidas — subtipos"], summary-cards, monthly-chart [always 12 months back + 3 months forward from the viewed reference month, independent of the page's period preset], category-pie [additive multi-select via use-category-filter.ts — expense + income donuts side by side; the sibling category-bars comparison chart was removed 2026-08-28 as duplicate data], expense-source-toggle [segments the expense donut only, by account type], budgets-panel [nests fixed expenses under their parent budget], transaction-explorer [account-type icon per row — no longer has its own "Reclassificar em lote" trigger, see below; below `sm:` renders a stacked card list instead of the table, mobile audit 2026-08-26], editable-category-cell — income-expense-chart removed 2026-08-14, see Implementation Status)
...
1b. "Despesas de {mês}" card (`MonthObligationsCard`) — donut com `total` no centro (= despesas realizadas do mês + o que falta pagar) + lista acionável do mês real: fatura de cada cartão (por competência), despesas programadas e dívidas OVERDUE_BILL/INSTALLMENT_PLAN ainda não pagas, cada uma com botão "Pagar". Independente do filtro de período. Substituiu o alerta "Vence essa semana" (2026-08-28)
...
3. Category Distribution, EXPENSE (donut, segmented by account type via `source` above) and INCOME (mirrored) — both always render regardless of the "Tipo" filter, side by side. The horizontal-bar "comparison" chart that used to sit next to each donut was removed 2026-08-28 (it duplicated the donut's data — see Implementation Status)
...
getCurrentMonthObligations() → MonthObligationsDTO
  -- NOVO (2026-08-28) — alimenta o card "Despesas de {mês}" do dashboard. Sempre ancorado no mês
  -- REAL corrente (nunca `filters`, igual OpenDebtsAlert). `total` = `paidTotal` + `remainingTotal`
  -- = despesas realizadas do mês (reconcilia com a DESPESAS do resumo, competência) + o que ainda
  -- falta pagar (despesas programadas / dívidas do mês não pagas — DESPESAS não conta até virarem
  -- lançamento). Cartão contado POR COMPETÊNCIA (getCardSummary → currentMonthInvoice /
  -- currentMonthPaidAmount), não pelo saldo em aberto — a 1ª versão usava getCardBalanceThroughMonth
  -- e subcontava o mês. Item "Fatura {cartão}" = currentMonthInvoice - currentMonthPaidAmount (> 0);
  -- a parte paga vai pra `paidTotal`. `paidTotal` = Σ transactions EXPENSE do mês + Σ
  -- currentMonthPaidAmount (CREDIT_CARD_PAYMENT NÃO é somado — dobraria contra a competência).
  -- `items` = só os não pagos. Auto-contida (faz seus fetches). Ver AI_CONTEXT.md "Despesas do mês
  -- (dashboard)".
```
...
// Card "Despesas de {mês}" do dashboard (getCurrentMonthObligations). Sempre o mês REAL corrente.
...
type MonthObligationsDTO = {
  month: string          // "YYYY-MM" — mês real corrente
