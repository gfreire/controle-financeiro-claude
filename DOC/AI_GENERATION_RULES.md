...
Required services: `dashboard.service.ts`, `transactions.service.ts`, `accounts.service.ts`, `cards.service.ts`, `reservoirs.service.ts`, `debts.service.ts`, `budgets.service.ts`, `fixed-expenses.service.ts`.
...
- `is_system` categories (`Juros`, `Rendimentos`, `Ajuste`) are never editable/deletable by any user and never copied — only consumed directly. `registerYield` creates transactions tagged `Rendimentos`; `reconcileAccountBalance` creates transactions tagged `Ajuste`. The choice between them is the user's, not an automatic classification — `Juros` is chosen manually on a normal entry (e.g. a card purchase for the invoice's interest line).
