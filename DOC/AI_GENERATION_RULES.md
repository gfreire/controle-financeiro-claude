...
- `is_system` categories (`Juros`, `Rendimentos`, `Ajuste`, `Estorno`, `Compras retroativas`) are never editable/deletable by any user, never copied, and **never selectable from a form** — they never appear in `CategorySelect` (the shared assignment picker), only in the dashboard/Cards/Transactions *filter* dropdowns. Each is applied exclusively by its own dedicated flow: `registerYield` → `Rendimentos`; `reconcileAccountBalance` → `Ajuste`; `registerInterest` ("Lançar Juros", account-side for `BANK`, and the Cards page's "Fatura" menu) → `Juros`; `refundCardPurchase`/`refundTransaction` → `Estorno`; the retroactive-purchase computed income → `Compras retroativas`. The choice of flow is the user's, not an automatic classification.
